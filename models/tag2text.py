...
class Tag2Text_Caption(nn.Module):
    ...
    def generate(self, image, sample=False, num_beams=3, max_length=30, min_length=10, top_p=0.9, repetition_penalty=1.0, tag_input = None, return_tag_predict = False):
        print('image',image.shape)#[9,360,640,3]
        image_embeds = self.visual_encoder(image)
        print('image_embeds',image_embeds.shape)#[9,145,1024]
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)#[9,145]
        ...
        if tag_input == None:
            ...
            logits = self.fc(mlr_tagembedding[0])#[9,3429]
            ...
            targets = torch.where(torch.sigmoid(logits) > self.class_threshold.to(image.device) , torch.tensor(1.0).to(image.device), torch.zeros(self.num_class).to(image.device))#【9，3429】
...
def init_tokenizer():
    tokenizer = BertTokenizer.from_pretrained('/root/media_hhd/data1/HKUST-GZ/Period/Video-LLaMA/bert-base-uncased')
