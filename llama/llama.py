...
class RMSNorm(torch.nn.Module): #均方根归一化
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ndim = x.ndim
    assert 0 <= 1 < ndim
    assert freqs_cis.shape == (x.shape[1], x.shape[-1])#[127,64]
    shape = [d if i == 1 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)]#[1,127,1,64]
...
) -> Tuple[torch.Tensor, torch.Tensor]:#每两个元素被解释为一个实部和虚部[2，127，32，128]->[2,127,32,64]
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2)) #[2,127,32,64]
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_) #[1,127,1,64]
    xq_out = torch.view_as_real(xq_ * freqs_cis).flatten(3)#[2,127,32,128]
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None):
        ...
        if not self.training:
            ...
            keys = self.cache_k[:bsz, : start_pos + seqlen] #可以提取之前存好的k v
        ...
        scores = torch.matmul(xq, keys.transpose(2, 3)) / math.sqrt(self.head_dim)#[2,32,127,127]
